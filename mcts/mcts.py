...
class MCTS(ABC):
  ...
  def __init__(self, initial_state: State, initial_player: int, exploration_parameter: float = math.sqrt(2.)):
    """
  ...
  def ucb_score(self, parent: Node, child: Node) -> float:
    ...
    return child.value() + self.exploration_parameter * math.sqrt(math.log(parent.visit_count) / child.visit_count)
  ...
  def select(self, node: Node) -> Tuple[Action, Node]:
    ...
    """
    candidates = [(self.ucb_score(node, child), action, child) for action, child in node.children.items()]

    max_score = max([candidate[0] for candidate in candidates])

    return random.choice([candidate for candidate in candidates if candidate[0] == max_score])[1:]
