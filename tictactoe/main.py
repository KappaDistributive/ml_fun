...
from tictactoe.game import TicTacToeEnv
...
class TicTacToeMCTS(MCTS):
  ...
  def _random_child(self, node: Node) -> Tuple[Action, Node]:
    ...
    if action not in node.children:
      ...
      node.children[action] = Node(to_play=-1 * node.to_play, state=deepcopy(observation[0]), reward=reward, is_terminal=done)
...
if __name__ == "__main__":
  ...
  while not done:
    if player == 1:
      ...
      print({action: child.value() for action, child in mcts.root.children.items()})
      action, _ = random.choice([(a, c) for a, c in mcts.root.children.items() if c.visit_count > 0 and c.value() == max_value])
