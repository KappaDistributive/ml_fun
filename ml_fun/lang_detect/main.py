...
class Embed(nn.Module):
    ...
    def forward(self, byte_ids: torch.LongTensor) -> torch.FloatTensor:
        ...
        h = torch.zeros(b, t, dtype=torch.int64, device=byte_ids.device)
...
class Net(nn.Module):
    def __init__(self, num_buckets: int = 256, embedding_dim: int = 64):
        super().__init__()
        self.embed = Embed(num_buckets, embedding_dim)

    def forward(self, byte_ids: torch.LongTensor) -> torch.FloatTensor:
        embedded = self.embed(byte_ids)  # (batch_size, max_len, embedding_dim)
        return embedded


def embed(texts: list[str], max_len: int = 512) -> torch.LongTensor:
    out = np.full((len(texts), max_len), fill_value=256, dtype=np.int64)
    for text_idx, text in enumerate(texts):
        if len(text) > max_len:
            print(
                f"Warning: Text {i} exceeds max length of {max_len} bytes and will be truncated."
            )
...
if __name__ == "__main__":
    ...
    # vec = embed(["Hello, world!", "This is a test."])
    # embed = Embed(4096,  64)
    # print(embed(vec).shape)
    # net = Net(256, 64)
    # print(net(vec).shape)
